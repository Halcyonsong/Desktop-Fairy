...
public interface ChatSummaryService {
    ...
    // 将一段历史消息压缩为摘要文本
    String summarizeMessages(List<ChatHistoryMessageVO> historyMessages, ModelConfig modelConfig);

    // 将指定会话的全部历史压缩为摘要文本
    String summarizeSessionHistory(String sessionId, ModelConfig modelConfig);

    // 仅压缩当前会话中“未压缩历史”，不足闸值则不压缩
    Optional<ChatSummaryVO> compressPendingHistory(String sessionId, ModelConfig modelConfig);
